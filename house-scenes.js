...
(() => {
 ...
 function seek(){if((innerWidth<=900||innerHeight<=600)&&!reduce.matches)return;if(video.readyState>=1&&!video.seeking&&Math.abs(video.currentTime-wantedTime)>.045)video.currentTime=wantedTime;}
 ...
 function render(now){
  ...
  const raw=progress(construction),a=clamp(raw/(innerWidth>900&&!reduce.matches?.38:1)),b=progress(orbit);
  ...
  if(Number.isFinite(video.duration)&&video.duration>0){
   const small=innerWidth<=900||innerHeight<=600;
